...
if (ICONIK_CUSTOM_ACTION_ID) {
  await iconikClient.patch(`/assets/v1/custom_actions/${iconikCustomActionConfig.context}/${ICONIK_CUSTOM_ACTION_ID}/`, iconikCustomActionConfig)
} else {
  const { data: { id } } = await iconikClient.post(`/assets/v1/custom_actions/${iconikCustomActionConfig.context}/`, iconikCustomActionConfig)
